# sigvcf/modules/proveedores/services.py
...
class ProveedorService:
    ...
    def consultar_ordenes_pendientes(self, proveedor_id: int) -> List[OrdenCompraProveedorDTO]:
        ...
        with self.uow:
            ...
            ordenes_dto = []
            # Estados que se consideran 'pendientes' para el proveedor
            estados_pendientes = ['APROBADA', 'FACTURA_CARGADA']

            for contrato in proveedor.contratos:
                for orden in contrato.ordenes_de_compra:
                    if orden.estado in estados_pendientes:
                        dto = OrdenCompraProveedorDTO(
                            id=orden.id,
                            codigo_licitacion_contrato=contrato.codigo_licitacion,
