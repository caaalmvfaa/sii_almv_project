...
class AlmacenViewModel(QObject):
    ...
    def actualizar_stock(self):
        ...
        try:
            ...
            self.operacion_finalizada.emit(f"Error al cargar el stock: {e}")
    ...
    def registrar_nueva_entrada(self, datos_entrada: Dict):
        ...
        try:
            ...
            self.operacion_finalizada.emit(f"Error al registrar la entrada: {e}")
    ...
    def despachar_por_qr(self, qr_id: str):
        ...
        if not qr_id or not qr_id.strip():
            self.operacion_finalizada.emit("El código QR no puede estar vacío.")
            return
        try:
            self.almacen_service.despachar_requerimiento(qr_id)
            self.operacion_finalizada.emit(f"Requerimiento '{qr_id}' despachado con éxito. El stock ha sido actualizado.")
            # Actualizar la vista de stock después de un despacho exitoso
            self.actualizar_stock()
        except Exception as e:
            self.operacion_finalizada.emit(f"Error en el despacho: {e}")