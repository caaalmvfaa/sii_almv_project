# sigvcf/modules/juridico/viewmodels.py
...
class JuridicoViewModel(QObject):
    ...
    def cargar_reportes_pendientes(self):
        ...
        try:
            ...
            self.operacion_finalizada.emit(f"Error al cargar reportes: {e}")
    ...
    def calcular_penalizacion(self, orden_id: int):
        ...
        """
        if orden_id <= 0:
            self.operacion_finalizada.emit("Por favor, introduzca un ID de Orden de Compra válido.")
            return
        try:
            resultado = self.juridico_service.calcular_penalizacion_por_atraso(orden_id)
            self.penalizacion_calculada.emit(resultado)
        except Exception as e:
            self.operacion_finalizada.emit(f"Error al calcular penalización: {e}")
    ...
    def registrar_incumplimiento(self, reporte_data: Dict):
        ...
        try:
            ...
            self.operacion_finalizada.emit("Reporte de incumplimiento registrado con éxito.")
            # Recargar la lista para que se refleje el nuevo reporte
            self.cargar_reportes_pendientes()
        except Exception as e:
            self.operacion_finalizada.emit(f"Error al registrar el reporte: {e}")